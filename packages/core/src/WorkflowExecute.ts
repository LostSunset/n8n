...
export class WorkflowExecute {
	...
	runPartialWorkflow2(
		...
		let startNodes = findStartNodes(subgraph, trigger, destinationNode, runData);
		assert.ok(startNodes.size > 0, 'we need a start node');
		...
		const { nodeExecutionStack, waitingExecution, waitingExecutionSource } =
			recreateNodeExecutionStack(subgraph, startNodes, newRunData, pinData ?? {});
