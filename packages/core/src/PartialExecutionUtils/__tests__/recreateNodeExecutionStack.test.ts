...
describe('recreateNodeExecutionStack', () => {
	...
	test.only('parent has multiple runs with data on the same output', () => {
		// ARRANGE
		const trigger = createNodeData({ name: 'trigger' });
		const node1 = createNodeData({ name: 'node1' });
		const graph = new DirectedGraph()
		...
		const runData: IRunData = {
			[trigger.name]: [
				toITaskData([{ data: { value: 'run1' } }]),
				toITaskData([{ data: { value: 'run2' } }]),
			],
		};
		const pinData: IPinData = {};
		const startNodes = [node1];
		...
		expect(nodeExecutionStack).toContainEqual({
			node: node1,
			data: { main: [[{ json: { value: 'run2' } }]] },
			source: {
				main: [{ previousNode: 'trigger', previousNodeOutput: 0, previousNodeRun: 1 }],
			},
		});
	...
	test('foo', () => {
		...
		const runData: IRunData = {
			[trigger.name]: [toITaskData([{ data: { value: 'trigger' } }])],
			//[loop.name]: [toITaskData([{ data: { value: 'loop' }, outputIndex: 1 }])],
			//[inLoop.name]: [toITaskData([{ data: { value: 'inLoop' } }])],
		};
		const pinData: IPinData = {};
		const startNodes = [loop];
		...
		expect(nodeExecutionStack[0]).toEqual({
			node: loop,
			data: { main: [[{ json: { value: 'trigger' } }]] },
			source: {
				main: [{ previousNode: 'trigger', previousNodeOutput: 0, previousNodeRun: 0 }],
			},
		});

		// FIXME: find out why the loop node is also added to the waiting
		// executions and fix it!
		expect(waitingExecution).toEqual({});
		expect(waitingExecutionSource).toEqual({});
	});
});
